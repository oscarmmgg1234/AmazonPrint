...
async function generateUPCLabels(product, quantity, opts = {}) {
  ...
  const {
    ...
    nameFontPt = 13,
    indexFontPt = 14,
    topBandPt = 20, // reserved height for top texts
    indexStart = 1,
  } = opts;
  ...
  for (let i = 0; i < quantity; i++) {
    ...
    // Name (top-right, cut after 35 chars, keep 21 front + 6 back)
    let displayName = product.name;
    if (displayName.length > 35) {
      const front = displayName.substring(0, 22).trim();
      const back = displayName.substring(displayName.length - 7).trim();
