...
class controller {
  ...
  async editProduct(product) {
    ...
    return { valid: true, message: "Product updated successfully." };
  }

  async removeProduct(productId) {
    await query_manager.raw(
      "DELETE FROM amazon_products WHERE internal_id = ?",
      [productId]
    );
    return { valid: true, message: "Product removed successfully." };
  } 
