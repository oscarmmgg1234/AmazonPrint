...
async function printUPCLabels(product, quantity, opts = {}) {
  ...
  await new Promise((resolve, reject) => {
    // 🧠 Redirect print job to Microsoft Print to PDF instead of Rollo
    const args = [
      `-print-to "Rollo Printer"`,
      `-print-settings "fit,center"`,
      `-paper-size "2x1in"`, // 🔥 tells Sumatra to send 2×1 media to spooler
      "-silent",
      "-exit-when-done",
      `"${pdfPath}"`,
    ];
    ...
    const child = spawn(sumatraPath, args, {
      shell: true,
      windowsHide: false, // show the Save dialog
    });
  ...
  // 🧹 Keep this disabled so you can inspect the file afterward
